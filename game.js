(() => {
  ...
  const SOLVE_LEVEL_1 = {
    name: 'Deur',
    machine: 'door',
    target: { x: LEVEL_1_SOLUTION_CHAIN.pink.x, teeth: 12, angle: LEVEL_1_SOLUTION_CHAIN.pink.angle },
    stock: [
  ...
  function buildLevel1Gear(id, x, y, teeth, angle, opts = {}){
    return {
      id,
      x,
      y,
  ...
  function buildLevel1SlotBaseGears(){
    return {
      green: { ...LEVEL_1_SOLUTION_CHAIN.green },
      pink: { ...LEVEL_1_SOLUTION_CHAIN.pink },
      blue: { ...LEVEL_1_SOLUTION_CHAIN.blue }
    }
  }
  ...
  function buildLevel1RackReference(){
    return {
      x: LEVEL_1_DOOR.rackClosedX,
      y: LEVEL_1_DOOR.rackY,
      pitchY: rackPitchLineY(),
      w: LEVEL_1_DOOR.rackW,
      h: LEVEL_1_DOOR.rackH
    }
  }

  function buildLevel1PinkFromRack(rack){
    const pink = buildLevel1Gear('target', LEVEL_1_PINK_X, rack.pitchY + gearRadii(12).pitchRadius, 12, 0, { fixed:true, target:true })
    alignGearToRackAtReference(pink, rack)
    return pink
  }

  function buildLevel1BlueFromPink(pink, greenTemplate){
    const blueTemplate = buildLevel1Gear('blue-slot', 0, 0, 12, 0)
    const points = circleIntersections(pink, meshDistance(pink, blueTemplate), greenTemplate, meshDistance(greenTemplate, blueTemplate))
    const point = points
      .filter(candidate => candidate.y >= LEVEL_1_DOOR.rackY + LEVEL_1_DOOR.rackH + blueTemplate.outerRadius + 28)
      .sort((a, b) => b.y - a.y)[0]
    const blue = { ...blueTemplate, x: point?.x ?? pink.x - meshDistance(pink, blueTemplate), y: point?.y ?? pink.y + meshDistance(pink, blueTemplate) }
    blue.angle = meshedGearAngleFor(pink, pink.angle, blue, blue.angle)
    return blue
  }

  function buildLevel1GreenFromBlue(blue, greenTemplate){
    const green = { ...greenTemplate }
    green.angle = meshedGearAngleFor(blue, blue.angle, green, green.angle)
    return green
  }

  function alignGearToRackAtReference(target, rack){
    target.y = rack.pitchY + target.pitchRadius
    const topAngle = -Math.PI / 2
    const gearPitch = TWO_PI / target.teeth
    const rackPhase = modulo((target.x - rack.x - RACK_TOOTH_PHASE) / TOOTH_PITCH, 1)
    target.angle = nearestMultiple(topAngle - modulo(rackPhase + .5, 1) * gearPitch, gearPitch, target.angle)
    target.rackBaseAngle = target.angle
  }

  function buildSlotFromBlueGear({ id, label, type, blue, green, pink, connectsGreen, connectsPink, result }){
    const slot = {
      id,
      label,
      type,
  ...
  function buildLevel1SolutionSlot(){
    const { green, pink, blue } = buildLevel1SlotBaseGears()
    const slot = buildSlotFromBlueGear({
      id: 'solution',
      label: 'oplossing',
      type: 'solution',
      blue,
    ...
    if(validateSolutionChain({ green, blue, pink })){
      slot.overallOk = true
      debugLevel1SlotBuild('solution-slot geldig', slot)
      return slot
    }

    debugLevel1SlotBuildReject('solution-slot', { ...slot, reason:'geen volledig geldige tandheugel-roze-blauw-groen ketting' })
    return null
  }

  function buildLevel1ExtraSlot({ id, label, type, anchorName, desiredAngle, result }){
    const { green, pink, blue } = buildLevel1SlotBaseGears()
    const anchor = anchorName === 'green' ? green : pink
    const blueGear = {
      ...blue,
      x: anchor.x + Math.cos(desiredAngle) * meshDistance(anchor, blue),
      y: anchor.y + Math.sin(desiredAngle) * meshDistance(anchor, blue)
    }
    blueGear.angle = meshedGearAngleFor(anchor, anchor.angle, blueGear, blue.angle)
    ...
    const slot = buildSlotFromBlueGear({
      ...
      green,
      pink,
      connectsGreen: anchorName === 'green',
      connectsPink: anchorName === 'pink',
      result
    })
    if(slot.overallOk) debugLevel1SlotBuild('extra slot geldig', slot)
    else debugLevel1SlotBuildReject(`extra slot ${id}`, slot)
    return slot
  }
  ...
  function evaluateLevel1SlotMesh(slot, preparedGears = null){
    const base = buildLevel1SlotBaseGears()
    const gearsForSlot = preparedGears || {
      green: { ...base.green, angle: slot.greenPhase ?? base.green.angle },
      blue: { ...base.blue, x: slot.x, y: slot.y, angle: slot.bluePhase ?? slot.phase ?? base.blue.angle },
      pink: { ...base.pink, angle: slot.pinkPhase ?? base.pink.angle }
    }
    const green = gearsForSlot.green
    const blue = gearsForSlot.blue
    const pink = gearsForSlot.pink
    const greenBlueBlockers = slot.type === 'greenOnly' ? [pink] : []
    const bluePinkBlockers = slot.type === 'pinkOnly' ? [green] : []
    const pinkRack = validatePinkRackMesh(pink)
    const bluePink = validateBluePinkMesh(blue, pink, bluePinkBlockers)
    const greenBlue = validateGreenBlueMesh(green, blue, greenBlueBlockers)
    const solutionChain = slot.type === 'solution' ? validateSolutionChain({ green, blue, pink }) : null
    const chainCanRotateOk = solutionChain?.chainCanRotateOk === true
    const doorCanOpenOk = solutionChain?.doorCanOpenOk === true
    const overallOk = slot.type === 'solution'
      ? solutionChain?.ok === true
      : slot.type === 'greenOnly'
        ? greenBlue.ok
        : slot.type === 'pinkOnly'
          ? bluePink.ok && pinkRack.ok
          : false

    return {
      greenBlueOk: greenBlue.ok,
      bluePinkOk: bluePink.ok,
      pinkRackOk: pinkRack.ok,
      chainCanRotateOk,
      doorCanOpenOk,
      overallOk,
      validationDetails: { greenBlue, bluePink, pinkRack, solutionChain }
    }
  }

  function validateFullMesh(slot){
    if(!slot) return false
    const validation = evaluateLevel1SlotMesh(slot)
    Object.assign(slot, validation)
    return slot.overallOk === true
  }

  function validatePinkRackMesh(pink){
    return strictRackContactStateForSlotBuild(pink)
  }

  function validateBluePinkMesh(blue, pink, blockers = []){
    return strictGearContactStateForSlotBuild(blue, pink, blockers)
  }

  function validateGreenBlueMesh(green, blue, blockers = []){
    return strictGearContactStateForSlotBuild(green, blue, blockers)
  }

  function validateSolutionChain({ green, blue, pink }){
    const pinkRack = validatePinkRackMesh(pink)
    const bluePink = validateBluePinkMesh(blue, pink)
    const greenBlue = validateGreenBlueMesh(green, blue)
    const chainCanRotateOk = pinkRack.ok && bluePink.ok && greenBlue.ok
    const rackCanMoveOk = chainCanRotateOk && !!pink?.target
    const doorCanOpenOk = rackCanMoveOk && LEVEL_1_DOOR.travel > 0
    return {
      ok: chainCanRotateOk && rackCanMoveOk && doorCanOpenOk,
      pinkRackOk: pinkRack.ok,
      bluePinkOk: bluePink.ok,
      greenBlueOk: greenBlue.ok,
      chainCanRotateOk,
      rackCanMoveOk,
      doorCanOpenOk,
      details: { pinkRack, bluePink, greenBlue }
    }
  }

  function strictGearContactStateForSlotBuild(anchor, child, otherGears = []){
    ...
    const blocked = otherGears.some(other => other && contactLineBlockedByGear(anchor, child, other))
    const overlaps = otherGears.some(other => other && gearsOverlapForSlotBuild(child, other))
    const clearanceBlocked = !child.target && child.y < Math.max(108 + child.outerRadius, LEVEL_1_DOOR.rackY + LEVEL_1_DOOR.rackH + child.outerRadius + 28)
  ...
  function strictRackContactStateForSlotBuild(target){
    if(!target) return { ok:false, reason:'missing target gear' }
    const rackX = LEVEL_1_DOOR.rackClosedX
    const pitchY = rackPitchLineY()
    const wantedY = pitchY + target.pitchRadius
    const insideRack = target.x >= rackX + TOOTH_PITCH && target.x <= rackX + LEVEL_1_DOOR.rackW - TOOTH_PITCH
    const gapError = Math.abs(target.y - wantedY)
    const gearTopPhase = toothPhaseAt(target, -Math.PI / 2, target.angle)
    const rackPhase = modulo((target.x - rackX - RACK_TOOTH_PHASE) / TOOTH_PITCH, 1)
    const phaseError = phaseDistance(gearTopPhase, rackPhase + .5)
    const geometryValid = insideRack && gapError <= STRICT_SLOT_RACK_GAP_TOLERANCE
    const phaseValid = phaseError <= STRICT_SLOT_PHASE_TOLERANCE
    return {
      ok: geometryValid && phaseValid,
      valid: geometryValid && phaseValid,
      geometryValid,
      phaseValid,
      gapError,
      phaseError,
      contact:{ x:target.x, y:pitchY }
    }
  }

  function gearsOverlapForSlotBuild(a, b){
    return dist(a, b) < visualCollisionRadius(a) + visualCollisionRadius(b) - 0.5
  }

  function contactLineBlockedByGear(anchor, loose, other){
    if(!anchor || !loose || !other || other.id === anchor.id || other.id === loose.id) return false
  ...
  function debugLevel1SlotBuild(message, slot){
    if(!DEBUG_SLOT_BUILD) return
    console.log(`[level 1 slot build] ${message}`, slotDebugSummary(slot))
  }

  function debugLevel1SlotBuildReject(message, slot){
    if(!DEBUG_SLOT_BUILD) return
    console.log(`[level 1 slot build] afgewezen: ${message}`, {
      ...slotDebugSummary(slot),
      reason: slot?.reason || rejectedSlotReasons(slot).join(', ')
    })
  }

  function slotDebugSummary(slot){
    if(!slot) return null
    return {
      id: slot.id,
      type: slot.type,
      x: slot.x,
      y: slot.y,
      bluePhase: slot.bluePhase,
      greenBlueOk: slot.greenBlueOk,
      bluePinkOk: slot.bluePinkOk,
      pinkRackOk: slot.pinkRackOk,
      chainCanRotateOk: slot.chainCanRotateOk,
      overallOk: slot.overallOk
    }
  }

  function rejectedSlotReasons(slot){
    if(!slot) return ['geen slot']
    const reasons = []
    if(slot.type === 'solution' && !slot.greenBlueOk) reasons.push('groen-blauw past niet exact')
    if(slot.type === 'solution' && !slot.bluePinkOk) reasons.push('blauw-roze past niet exact')
    if(slot.type === 'solution' && !slot.pinkRackOk) reasons.push('roze-tandheugel past niet exact')
    if(slot.type === 'greenOnly' && !slot.greenBlueOk) reasons.push('groen-blauw past niet exact')
    if(slot.type === 'pinkOnly' && !slot.bluePinkOk) reasons.push('blauw-roze past niet exact')
    if(!slot.chainCanRotateOk && slot.type === 'solution') reasons.push('ketting kan niet draaien')
    if(!slot.overallOk) reasons.push('overallOk is false')
    return reasons
  }
  ...
  function meshDistance(a, b){ return a.pitchRadius + b.pitchRadius }
  function rackXForProgress(t = doorProgress){ return LEVEL_1_DOOR.rackClosedX - LEVEL_1_DOOR.travel * t }
  function rackPitchLineY(){ return LEVEL_1_DOOR.rackY + RACK_BODY_H + TOOTH_DEDENDUM }
  function rackDriveZone(t = doorProgress){
    const door = LEVEL_1_DOOR
    const target = getGear('target')
    const gearY = rackPitchLineY() + (target?.pitchRadius || gearRadii(SOLVE_LEVEL_1.target.teeth).pitchRadius)
    const x = rackXForProgress(t)
    return { x, y:rackPitchLineY() - 8, w:door.rackW, h:TOOTH_DEPTH + 24, gearY }
  }

  function modulo(v, m){ return ((v % m) + m) % m }
  function nearestMultiple(value, step, nearValue){
    return value + Math.round((nearValue - value) / step) * step
  }
  function toothPhaseAt(gear, worldAngle, gearAngle = gear.angle){
    return modulo((worldAngle - gearAngle) / (TWO_PI / gear.teeth), 1)
  }
  function phaseDistance(a, b){
    const d = Math.abs(modulo(a - b + .5, 1) - .5)
    return d
  }

  function nearestValleyAngle(anchor, angle){
    const pitch = TWO_PI / anchor.teeth
    return anchor.angle + nearestMultiple(pitch * .5, pitch, angle - anchor.angle)
  }
  ...
  function meshedGearAngleFor(anchorGear, anchorAngle, childGear, childAngle = childGear.angle){
    ...
    const childContactAngle = contactAngle + Math.PI
  ...
  function meshedGearAngle(anchorGear, childGear){
    return meshedGearAngleFor(anchorGear, anchorGear.angle, childGear)
  }
  ...
  function rackGearPhaseError(target){
    const rackX = rackXForProgress()
    const gearTopPhase = toothPhaseAt(target, -Math.PI / 2)
    const rackPhase = modulo((target.x - rackX - RACK_TOOTH_PHASE) / TOOTH_PITCH, 1)
    return phaseDistance(gearTopPhase, rackPhase + .5)
  }

  function rackMeshState(target){
    if(!target) return { valid:false, geometryValid:false, phaseValid:false, gapError:0 }
    const rackX = rackXForProgress()
    const pitchY = rackPitchLineY()
    const wantedY = pitchY + target.pitchRadius
    const insideRack = target.x >= rackX + TOOTH_PITCH && target.x <= rackX + LEVEL_1_DOOR.rackW - TOOTH_PITCH
    const gapError = Math.abs(target.y - wantedY)
    const phaseError = rackGearPhaseError(target)
    const geometryValid = insideRack && gapError <= 1.5
    const phaseValid = phaseError < .08
    return {
      valid: geometryValid && phaseValid,
      geometryValid,
      phaseValid,
      geometryError: geometryValid ? 0 : Math.hypot(target.x - clamp(target.x, rackX, rackX + LEVEL_1_DOOR.rackW), target.y - wantedY),
      gapError,
      phaseError,
      contact:{ x:target.x, y:pitchY }
    }
  ...
  function alignTargetGearToRack(target){
    if(!target) return
    target.y = rackPitchLineY() + target.pitchRadius
    const topAngle = -Math.PI / 2
    const gearPitch = TWO_PI / target.teeth
    const rackPhase = modulo((target.x - rackXForProgress() - RACK_TOOTH_PHASE) / TOOTH_PITCH, 1)
    target.angle = nearestMultiple(topAngle - modulo(rackPhase + .5, 1) * gearPitch, gearPitch, target.angle)
    target.rackBaseAngle = target.angle
  }
  ...
  function buildSolveConstraintGraph({ applyAngles = false } = {}){
    ...
    candidates.forEach(link => {
      ...
      if(!a || !b || !gearMeshPhaseFits(a, b)){
        rejected.push({ a:link.a, b:link.b })
        return
      }
  ...
  function closestLevel1Slot(gear){
    return LEVEL_1_SLOTS
      .filter(slot => slot.overallOk === true)
  ...
  function seedSolveSnapPhases(gear, anchors){
    const target = anchors.find(anchor => anchor.target)
    const poweredAnchor = anchors.find(anchor => drivenGearIds().has(anchor.id))
    if(target){
      alignTargetGearToRack(target)
      alignGearToGearMesh(target, gear)
      if(poweredAnchor && poweredAnchor.id !== target.id) alignGearToGearMesh(gear, poweredAnchor)
      return
    }
    if(poweredAnchor) alignGearToGearMesh(poweredAnchor, gear)
    else if(anchors[0]) alignGearToGearMesh(anchors[0], gear)
  }
  ...
  function snapToSolveConstraintPosition(gear){
    ...
    for(const position of positions){
      ...
      gear.x = position.x
      gear.y = position.y
      alignTargetGearToRack(getGear('target'))
